...
# Function to reset test directories
reset_directories() {
    rm -rf $DEST_DIR_DOWNLOAD/*
}

# Download tests with options
echo -e "\nProcessing directory: $SOURCE_DIR_DOWNLOAD with options"
reset_directories
for LOCAL_FILE in $SOURCE_DIR_DOWNLOAD/*; do
    REMOTE_FILE="$DEST_DIR_DOWNLOAD/$(basename $LOCAL_FILE)"
    perform_test "$LOCAL_FILE" "$REMOTE_FILE" "--option blksize=800 --option timeout=3" "download"
done

sleep 1  # Give a moment before the next set of tests

# Download tests without options
echo "Processing directory: $SOURCE_DIR_DOWNLOAD without options"
reset_directories
for LOCAL_FILE in $SOURCE_DIR_DOWNLOAD/*; do
    REMOTE_FILE="$DEST_DIR_DOWNLOAD/$(basename $LOCAL_FILE)"
    perform_test "$LOCAL_FILE" "$REMOTE_FILE" "" "download"
done
...
compare_directories $SOURCE_DIR_DOWNLOAD $DEST_DIR_DOWNLOAD
